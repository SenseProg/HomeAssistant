...
| `bez_svitla_vymknuty_vazhke` | switches off boiler, floor heating, dryer, small washer socket, irrigation pump, PRANA heater, Blauberg/PRANA fans, closes the EV charging session when the grid drops, and again when someone turns one back on; gated by `input_boolean.bez_svitla_vymykaty`. Since 2026-09-03 it does NOT touch the well pump, and records what it switched off as keys in `input_text.bez_svitla_vymkneno` | LocalTuya / Terneo / PRANA reachability |
| `bez_svitla_nasos_sverdlovyny_batareia` | well pump off only when, on battery, `inverter_battery` drops below `input_number.nasos_sverdlovyny_porih_batarei` (owner: 5–10 %); adds key `nasos` to the memory | `inverter_battery` |
| `svitlo_zyavylos_uvimknuty_nazad` | grid back for 5 min (or HA start + 5 min): switches back on exactly the keys in the memory (boiler, floor, dryer, washer socket, PRANA heater, Blauberg, PRANA via `rest_command.prana_start`, well pump, EV session), clears the memory, pushes the list; gated by `input_boolean.pislia_svitla_vmykaty` (default on). Irrigation pump is never restored - its schedule owns it | the memory helper |
| `svitlo_zyavylos_nahadaty_boiler` | reminder to re-enable the boiler after the grid returns - only while `pislia_svitla_vmykaty` is off | — |
| `boiler_babusi_heat_start` / `ev_schedule_window_start` / `ev_schedule_car_plugged_inside_window` | night schedules (2 kW boiler at 04:00, 7 kW charger at 23:00) require `svitlo_vidsutnie` off since 2026-09-03; if the grid returns inside the window they start late (`for: 5 min` trigger + time condition on the window helpers) | — |
