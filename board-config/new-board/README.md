# Production replacement board — running the house since 2026-08-13

The owner identifies the current physical replacement as RK3588. The migrated
Linux runtime at `192.168.50.141` still reports `Forlinx OK3568-C Board`,
`rockchip,rk3568`, and the legacy hostname `ok3568`. Both facts are recorded
here deliberately: `.141` is the production target, while the stale runtime
labels must not be used to select a different host.
