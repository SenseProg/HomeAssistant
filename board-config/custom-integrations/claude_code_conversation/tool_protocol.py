...
def local_tool_instructions(tools: tuple[dict[str, str], ...]) -> str:
    """Return the always-available half of the tool protocol.

    Читання історії та атрибутів плюс ведення реєстру інцидентів. Жоден із цих
    інструментів не чіпає пристрої, тому вони доступні незалежно від того,
    дозволено керування чи ні. Вони існують саме для того, щоб агент не
    ...
    return (
        "\n\nДАНІ Й ЖУРНАЛ ПРОБЛЕМ.\n"
        "Крім готових блоків у запиті, тобі доступні інструменти нижче. Вони "
        "нічого не вмикають і не змінюють у будинку:\n\n"
        f"{lines}\n\n"
        "Виклик - РІВНО один рядок JSON і нічого більше:\n"
        '{"tool_call": {"name": "НАЗВА", "arguments": {…}}}\n\n'
        "Читання викликай лише тоді, коли наданих блоків справді бракує: інша "
