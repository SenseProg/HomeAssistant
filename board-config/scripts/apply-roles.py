...
#
# Звіряти саме з іменем облікового запису, а не з іменем особи: 2026-08-10
# власника перейменували «Дім Ковтуни» -> «Дмитро», і скрипт мовчки перестав
# його бачити. Логін при цьому лишився `дім`.
MATRIX = {
    # Обидва адміністратори мають входити ззовні через Tailscale, а адреса
    # 100.x для HA НЕ локальна (перевірено його ж is_local), тож local_only
    # для них означав би «ззовні не пускати взагалі». Решта ролей лишається
    # локальною: віддалений доступ отримують рівно ті, кому він потрібен.
    'Дмитро':      ('admin', False),     # власник, логін `дім`
    'Роман':       ('admin', False),     # другий адміністратор, логін `роман`
    'Катерина':    ('user', True),
    'Олеся':       ('user', True),
