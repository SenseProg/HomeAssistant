...
2026-09-03: the clips are no longer exposed under www. Until now
www/motion-clips was a second mount of the same NAS folder so that /local/
could serve the files without a token - and Home Assistant Core archives the
whole config tree, mounts included: the nightly backup carried every clip
(5-9 GB) and aborted whenever a clip was renamed mid-tar, so no backup had
completed since 10.08. The list now carries media-source ids
(media-source://media_source/local/video/ha-motion/<day>/<file>) and the page
asks Home Assistant for a signed URL per clip through the parent window's
`hass.callWS({type: "media_source/resolve_media"})` - the iframe is same-origin
(sandbox allows allow-same-origin allow-scripts). A signature is valid for
24 hours; it is requested lazily, when a tile scrolls into view.
"""
import json
import os
import re

CLIPS = '/userdata/hass/config/media/video/ha-motion'
MEDIA_ID = 'media-source://media_source/local/video/ha-motion'
...
// Кліпи лежать на NAS поза www: Home Assistant віддає їх лише з підписом
// (/media/local/...?authSig=...), а підпис дає media_source/resolve_media.
// Сторінка живе в iframe того самого origin, тож бере hass у батьківського
// вікна. Підпис чинний 24 години; кешуємо на 20.
function getHass(){
  try {
    const el = window.parent && window.parent.document.querySelector('home-assistant');
    return el && el.hass ? el.hass : null;
  } catch(e) { return null; }
}
const urlCache = new Map();
async function signedUrl(mid){
  const hit = urlCache.get(mid);
  if(hit && Date.now() - hit.at < 20 * 3600 * 1000) return hit.url;
  const hass = getHass();
  if(!hass) throw new Error('немає hass');
  const r = await hass.callWS({type: 'media_source/resolve_media', media_content_id: mid});
  urlCache.set(mid, {url: r.url, at: Date.now()});
  return r.url;
}
function attach(v){
  if(v.dataset.state === 'loading' || v.dataset.state === 'ready') return;
  v.dataset.state = 'loading';
  signedUrl(v.dataset.mid).then(function(url){
    v.src = url; v.preload = 'metadata'; v.dataset.state = 'ready';
  }).catch(function(e){
    v.dataset.state = 'error';
    const cap = v.nextElementSibling;
    if(cap) cap.innerHTML += ' <span class="err">· не вдалося підписати посилання</span>';
  });
}
...
// into view. The same moment is when the signed URL is requested.
const seenObserver = ('IntersectionObserver' in window)
  ? new IntersectionObserver(function(entries, obs){
      for(const e of entries){
        if(e.isIntersecting){ attach(e.target); obs.unobserve(e.target); }
      }
    }, {rootMargin: '400px'})
  : null;
...
function render(){
  ...
  if(!getHass()){
    grid.innerHTML = '<div class="empty">Галерея відкривається лише всередині Home Assistant (вкладка «Камера і рух»): ' +
      'посилання на ролики підписуються через сесію дашборда.</div>';
    return;
  }
  ...
  for(const c of sel){
    ...
    v.controls = true; v.preload = 'none'; v.dataset.mid = c.m;
    v.addEventListener('error', function(){
      // прострочений підпис або NAS моргнув - один повтор із новим підписом
      if(v.dataset.retried) return;
      v.dataset.retried = '1'; v.dataset.state = ''; urlCache.delete(c.m); attach(v);
    });
    const cap = document.createElement('figcaption');
    cap.textContent = c.t;
    fig.append(v, cap); frag.append(fig);
    if(seenObserver){ seenObserver.observe(v); } else { attach(v); }
  }
  grid.append(frag);
}
...
async function load(){
  ...
  const sig = fresh.length + '|' + (fresh[0] ? fresh[0].m : '');
